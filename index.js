...
if (argv._.length === 0) {
  ...
  for (const postOrImage of argv._) {
    if (fs.existsSync(postOrImage)) {
      // Currently the only supported embed is images.
  ...
  try {
    ...
    async function post_to_X(text, images, parent_post) {
      const mediaIds = await Promise.all(
        images.map((image) => consumerClient.v1.uploadMedia(image))
      );
...
if (bluesky) {
  ...
  try {
    ...
    async function post_to_bsky(text, images, parent_id, root_id) {
      ...
      if (images.length > 0) {
        let image_response = await Promise.all(
          images.map((image) => {
        ...
        post_data.embed = {
          ...
          images: image_response.map((image_res) => {
            return { image: image_res.data.blob, alt: '' };
          }),
        };
      }
    ...
    for (const post of thread) {
      ...
      previous_response = await post_to_bsky(
        post.text,
        post.images_jpg,
        previous_response,
        head_response
...
if (farcaster) {
  ...
  try {
    ...
    for (const post of thread) {
      ...
      let farcasterProcess = spawnSync(
        `${dirname}/farcaster_poster.py`,
