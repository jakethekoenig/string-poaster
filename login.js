...
if (xyn == 'y' || xyn == 'Y' || xyn == '') {
  console.log(
    'Currently OAuth is not supported so you will have need your own twitter developer account and app credentials.'
  );
  console.log(
    'See the following for more information: https://developer.twitter.com/en/docs/authentication/oauth-1-0a/api-key-and-secret'
  );
...
if (byn == 'y' || byn == 'Y' || byn == '') {
  console.log(
    'Bluesky is authenticated with username and password. Note you can setup an application specific password which is easier to revoke.'
  );
...
if (tyn == 'y' || tyn == 'Y' || tyn == '') {
  console.log(
    'Threads is authenticated with username and password. They are used by this script to obtain a user token and the password is not saved.'
  );
...
if (myn == 'y' || myn == 'Y' || myn == '') {
  ...
  let clientSecret;
  console.log(
    'Mastodon is authenticated with an access token. You will have to login to your Mastodon with the provided url and enter the code they give you. (Note the developers have observed problems with some instances. But mastodon.social works.'
  );
